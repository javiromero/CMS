...
class SubscriptionBase(models.Model):
    ...
    subscribed = models.BooleanField(_('Suscrito'), default=True)
    email = models.EmailField(_('Correo'), unique=True)
    created_on = models.DateField(_("Creado el"), blank=True)
    updated_on = models.DateField(_("Actualizado el"), blank=True)
    
    class Meta:
        abstract = True
        verbose_name = _(u"Suscripcion a la Lista de correo")
        verbose_name_plural = _(u"Suscripciones a la Lista de correo")
